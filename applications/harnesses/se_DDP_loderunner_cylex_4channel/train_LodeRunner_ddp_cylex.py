...
from yoke.datasets.load_npz_dataset_mpp_cylex import TemporalDataSet
...
parser.add_argument("--warmup_lr", type=float, default=None,
                    help="Override anchor_lr during frozen phase (scheduler anchor lr).")
...
def _freeze_backbone_inplace(model: torch.nn.Module) -> None:
    """Freeze all params, then unfreeze variable-embedding + head modules if present."""
    _set_requires_grad(model, False)
    for attr in ("parallel_embed", "var_embed_layer", "linear4unpatch"):
        if hasattr(model, attr):
            _set_requires_grad(getattr(model, attr), True)

def _unfreeze_all_inplace(model: torch.nn.Module) -> None:
    _set_requires_grad(model, True)

def _maybe_get_base_model(model: torch.nn.Module) -> torch.nn.Module:
    # DDP wraps the model at model.module
...
def main(args, rank, world_size, local_rank, device):
    ...
    model_args = {
        ...
        #    "pressure_Water",
        #],
        "default_vars": ['Uvelocity', 'Wvelocity', 'av_density', 'density_maincharge'],
        "image_size": (1120, 400),
        "patch_size": (10, 5),
        "embed_dim": embed_dim,
        "emb_factor": 2,
    ...
    if CONTINUATION:
        ...
        # Freeze/unfreeze backbone based on upcoming epoch when chaining jobs.
        freeze_epochs = getattr(args, "freeze_backbone_epochs", 0) or 0
        # This job will start training at epoch (starting_epoch + 1) due to starting_epoch += 1 later.
        next_epoch = starting_epoch + 1
        base_model = _maybe_get_base_model(model)
        if freeze_epochs > 0 and next_epoch <= freeze_epochs:
            _freeze_backbone_inplace(base_model)
            print(f"Backbone frozen for epoch {next_epoch} (<= {freeze_epochs}).")
        else:
            _unfreeze_all_inplace(base_model)
            if freeze_epochs > 0:
                print(f"Backbone unfrozen for epoch {next_epoch} (> {freeze_epochs}).")
    else:
        # Initialize model and optimizer state.
        # If not continuing, set starting_epoch to 0.
        starting_epoch = 0
        model = LodeRunner(**model_args)
        # Optional: initialize from a pretrained checkpoint/state_dict (weights-only).
        if args.pretrained_model is not None:
            ...
            for k, v in state_dict.items():
                ...
                cleaned[nk] = v
            # Shape-safe load: skip mismatched tensors
            ...
            print(f"Pretrained load complete: loaded {len(filtered)} tensors; skipped {skipped}.")
        ...
        model.to(device)
        # Freeze/unfreeze backbone for initial epochs if requested.
        freeze_epochs = getattr(args, "freeze_backbone_epochs", 0) or 0
        base_model = _maybe_get_base_model(model)
        # starting_epoch is 0 here; this job will train epoch 1 first.
        if freeze_epochs > 0 and 1 <= freeze_epochs:
            _freeze_backbone_inplace(base_model)
            print(f"Backbone frozen for epoch 1 (<= {freeze_epochs}).")
        else:
            _unfreeze_all_inplace(base_model)
    ...
    #
    # # For multi-node
    # Allow overriding anchor LR for fine-tuning / warmup-freeze phases.
    effective_anchor_lr = args.anchor_lr
    # If the upcoming epoch is within the frozen phase, optionally use warmup_lr.
    freeze_epochs = getattr(args, "freeze_backbone_epochs", 0) or 0
    upcoming_epoch = starting_epoch + 1 if CONTINUATION else 1
    if freeze_epochs > 0 and upcoming_epoch <= freeze_epochs and getattr(args, "warmup_lr", None) is not None:
    ...
    ddp_anchor_lr = effective_anchor_lr * lr_scale / original_batchsize
    #
    # For single node
    ...
    train_dataset = TemporalDataSet(
        ...
        max_file_checks=10,
        half_image=True,
        active_fields=["av_density", "Uvelocity", "Wvelocity", "density_maincharge"]
    )
