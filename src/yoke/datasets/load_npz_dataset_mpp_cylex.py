...
str='velocity', thermodynamic_variables: str='density', custom_active_fields: list[str]=None) -> None:
        ...
        self.custom_active_fields = custom_active_fields
...
#'energy_Void', 'pressure_Void', 'density_Water', 'energy_Water', 'pressure_Water']
            ...
            self.all_hydro_field_names = ['Uvelocity', 'Wvelocity', 'av_density', 'density_maincharge']
...
['energy_booster'])
        ...
        if getattr(self, 'custom_active_fields', None) is not None:
            # Force active lists to the user-specified order.
            custom = list(self.custom_active_fields)
            self.active_npz_field_names = np.array(custom, dtype=object)
            self.active_hydro_field_names = np.array(custom, dtype=object)
...
max_timeIDX_offset: int, max_file_checks: int, half_image: bool=True, active_fields: list[str]=None) -> None:
        ...
        while prefix_attempt < 5:
            ...
            while attempt < self.max_file_checks:
                ...
                try:
                    ...
                    continue
                try:
                    ld = LabeledData(self.npz_dir + start_file, self.csv_filepath, custom_active_fields=self.active_fields)
...
self.csv_filepath, custom_active_fields=self.active_fields).get_active_npz_field_names()
                active_hydro_field_names = LabeledData(file_path,
self.csv_filepath, custom_active_fields=self.active_fields).get_active_hydro_field_names()
                channel_map = LabeledData(file_path, self.csv_filepath, custom_active_fields=self.active_fields).get_channel_map()
                field_imgs = []
